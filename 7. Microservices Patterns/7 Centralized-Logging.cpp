...
1: Log Producers
    → Microservices write logs (file, console, or structured JSON)

2: Log Aggregator / Collector
    → Collects logs from services
    → Examples: Filebeat, Fluentd, Logstash

3: Log Storage & Index
    → Stores logs for search & analysis
...
→ Service Logs → Log Collector → Elasticsearch → Kibana/Grafana

------------------------------------------------------------------------------------------------------------------------------

Logback.xml:
    → It is used to configure logging in Spring Boot applications.
    → Used to save logs in external files 
    → Used to set log levels
